"""
SWING STRATEGY ENGINE v3.9 - FIXED ADX CALCULATION
═══════════════════════════════════════════════════════════════════════════════

v3.9 FIX (Feb 25, 2026):
- CRITICAL: "Simplified ADX" was NOT ADX at all. Formula was 100*ATR/price
  which equals 1-5 for S&P 500 stocks. max(10,...) floored ALL stocks to 10.
  Threshold of 12 meant 100% of stocks failed. Replaced with REAL ADX
  calculation (Wilder's directional movement method).
- Real ADX range: 0-100. Threshold set to 15 (was 12 on broken scale).
  ADX > 15 = stock has directional trend. ADX > 25 = strong trend.

v3.8 FIXES (Feb 24, 2026) - RETAINED:
1. Removed inverted "not at peak" momentum check
2. Raised confidence base, added boost conditions
3. Granular filter stats for reporter
4. Relaxed momentum thresholds
...
class SwingStrategyEngine:
    ...
    def __init__(self, config: Dict, risk_manager=None, ml_engine=None, 
        ...
        pprint(f"")
        pprint(f"[INIT] SwingStrategyEngine v3.9 - FIXED ADX CALCULATION")
        pprint(f"[INIT] ═══════════════════════════════════════════════════")
        pprint(f"[INIT] 🔧 FIX: Replaced broken ADX formula with real Wilder ADX")
        pprint(f"[INIT] 🔧 FIX: Old formula: 100*ATR/price → always 10 → always < 12")
        pprint(f"[INIT] 🔧 FIX: New formula: Real ADX 0-100 scale, threshold 15")
    ...
    def generate_signals(self, universe: List[str]) -> List[Dict]:
        ...
        pprint("")
        pprint("=" * 70)
        pprint(f"  CYCLE {self.cycle_count} | {et.strftime('%H:%M:%S')} ET | v3.9 ADX-FIXED")
