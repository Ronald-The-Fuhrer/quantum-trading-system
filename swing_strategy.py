...
class SwingStrategyEngine:
    ...
    def can_buy_new_position(self) -> Tuple[bool, str]:
        ...
        v10.4: Only count positions bought today that are STILL HELD.
        Completed round trips already consumed their day trades."""
        self._check_new_day()
        
        day_trades_remaining = self.get_day_trades_remaining()
        held_today = [t for t in self.positions_bought_today if t in self.current_positions]
        positions_needing_daytrade = len(held_today)
        
        if positions_needing_daytrade >= day_trades_remaining:
            return False, f"PDT_LIMIT (holding {positions_needing_daytrade} today positions, only {day_trades_remaining} day trades left)"
        
        return True, f"OK ({positions_needing_daytrade}/{day_trades_remaining} slots used)"
    ...
    def generate_signals(self, universe: List[str], alpaca_remaining: int = None) -> List[Dict]:
        et = datetime.now(EST)
        self.cycle_count += 1
        self._check_new_day()
        self._cleanup_expired_blocks()
        
        # v10.3: Use Alpaca day trade count as source of truth
        # Internal pdt_tracker was corrupted by ghost triple-sells on Mar 13
        # Alpaca rolling 5-day window is the ONLY reliable count
        if alpaca_remaining is not None:
            day_trades_remaining = alpaca_remaining
        else:
            # Fallback to internal tracker if Alpaca count not provided
            day_trades_remaining = self.get_day_trades_remaining()
        
        # v10.4: Count only positions bought today that are STILL HELD.
        # These are the ones that will need a day trade to sell at EOD.
        # Alpaca's remaining count already reflects completed round trips
        # (ARE bought+sold = 1 used, BX bought+sold = 1 used, both done).
        # Old bug: counted ALL buys today (2) vs remaining (1) → blocked,
        # even though both positions were already sold and 1 day trade was free.
        held_today_positions = [t for t in self.positions_bought_today if t in self.current_positions]
        positions_needing_daytrade = len(held_today_positions)
        
        # Can we buy if: remaining day trades > positions that still need selling
        if positions_needing_daytrade >= day_trades_remaining:
            can_buy = False
            buy_status = f"PDT_LIMIT (holding {positions_needing_daytrade} today positions, only {day_trades_remaining} day trades left)"
        else:
            can_buy = True
            buy_status = f"OK ({positions_needing_daytrade}/{day_trades_remaining} slots used)"
        ...
        pprint("")
        pprint("=" * 70)
        pprint(f"  CYCLE {self.cycle_count} | {et.strftime('%H:%M:%S')} ET | v4.2")
        pprint(f"  {market_icon} SPY: {market_pct*100:+.2f}% | 💰 P&L: ${self.daily_realized_profit:+.2f}")
        pprint(f"  📊 Day Trades: {day_trades_remaining}/3 remaining (Alpaca)")
        ...
        if entries_blocked:
            ...
            pprint(f"        Slots available: {day_trades_remaining - positions_needing_daytrade}")
            entry_signals = self._generate_entry_signals(universe, day_trades_remaining - positions_needing_daytrade)
