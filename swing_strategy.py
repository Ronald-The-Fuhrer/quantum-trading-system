"""
SWING STRATEGY ENGINE v3.8 - FIXED ZERO SIGNALS BUG
═══════════════════════════════════════════════════════════════════════════════

v3.8 FIXES (Feb 24, 2026):
1. FIXED: Momentum Check 4 was INVERTED - rejected stocks NEAR high instead
   of FAR from high. Stocks meeting uptrend checks 1-3 are always near their
   daily high, making Check 4 contradict checks 1-3. REMOVED Check 4 entirely.
2. FIXED: Confidence base 0.82 with floor 0.85 was too tight. Raised base to
   0.83 and added more boost conditions (MACD bullish, near SMA support).
3. FIXED: Reporter stats keys didn't match - now sends granular filter stats
   (price_filtered, adx_filtered, etc.) so the report shows WHERE stocks fail.
4. RELAXED: Momentum thresholds slightly - green today +0.3% (was +0.5%),
   30-min gain +0.1% (was +0.2%).
...
class SwingStrategyEngine:
    ...
    def __init__(self, config: Dict, risk_manager=None, ml_engine=None, 
        ...
        pprint(f"")
        pprint(f"[INIT] SwingStrategyEngine v3.8 - FIXED ZERO SIGNALS BUG")
        pprint(f"[INIT] ═══════════════════════════════════════════════════")
        pprint(f"[INIT] 🔧 FIX: Removed inverted 'not at peak' momentum check")
        pprint(f"[INIT] 🔧 FIX: Relaxed momentum thresholds (green +0.3%, 30m +0.1%)")
        pprint(f"[INIT] 🔧 FIX: Confidence base raised, more boost conditions")
        pprint(f"[INIT] 🔧 FIX: Granular filter stats for reporter")
    ...
    def generate_signals(self, universe: List[str]) -> List[Dict]:
        ...
        pprint("")
        pprint("=" * 70)
        pprint(f"  CYCLE {self.cycle_count} | {et.strftime('%H:%M:%S')} ET | v3.8 FIXED")
    ...
    def _generate_entry_signals(self, universe: List[str], max_entries: int) -> List[Dict]:
        ...
        pprint(f"")
        pprint(f"        ╔═══════════════════════════════════════╗")
        pprint(f"        ║  SCAN RESULTS v3.8 FIXED              ║")
        pprint(f"        ╠═══════════════════════════════════════╣")
    ...
    def _evaluate_ticker(self, ticker: str, stats: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            # ADX boost (strong directional movement)
            if adx > 25:
                confidence += 0.03
            elif adx > 18:
                confidence += 0.01  # NEW: small boost for moderate ADX
