"""
QUANTUM TRADING SYSTEM v10.2 - PDT COUNTDOWN + SMART SIZING + ATR FILTER
═══════════════════════════════════════════════════════════════════════════════

v10.2 FIX (Mar 4, 2026):
 1. PDT COUNTDOWN: 100% Alpaca-sourced. Queries broker's fill history
    via get_activities(FILL) to find actual day trade dates. Calculates
    exact expiry from Alpaca's rolling 5-business-day window. Zero
    dependency on pdt_state.json. Shows at startup + every 30 cycles
    when PDT is blocking entries. Includes "next slot opens" and
    "full reset" dates.
...
class QuantumSwingTrader:
    """v10.2 - PDT COUNTDOWN + SMART SIZING + ATR FILTER"""
    
    def __init__(self, config_path: str = 'config.json'):
        pprint("=" * 80)
        pprint("   QUANTUM TRADING SYSTEM v10.2 - PDT COUNTDOWN + SMART SIZING + ATR FILTER")
    ...
    def _print_startup_info(self):
        ...
        # v10.2: PDT countdown — when do trades expire from rolling window?
        pprint("")
        for line in self._get_pdt_countdown_display():
            pprint(f"  {line}")
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # v10.2: PDT COUNTDOWN — 100% ALPACA-SOURCED
    # Queries broker's actual fill history to find day trades and their dates.
    # No dependency on pdt_state.json (which can be stale/empty).
    ...
    @staticmethod
    def _count_business_days_between(from_date, to_date) -> int:
    ...
    def _get_day_trades_from_alpaca(self) -> list:
        """Query Alpaca's ACTUAL fill history to identify day trades.
        
        A day trade = buying AND selling the same ticker on the same calendar day.
        We scan the last 10 calendar days of fills to cover the 5-business-day window.
        
        Returns list of dicts: [{'ticker': str, 'trade_date': date}, ...]
        Returns empty list on error (caller should check alpaca_count as fallback).
        """
        try:
            broker = self.execution_engine.broker
            if not broker:
            ...
            # Get fills from last 10 calendar days (covers weekends + rolling window)
            lookback_start = get_eastern_time() - timedelta(days=10)
            
            activities = broker.get_activities(
                activity_types='FILL',
                after=lookback_start.isoformat()
            )
            
            if not activities:
                return []
            
            # Group fills by (date, symbol, side)
            # Each fill has: symbol, side, transaction_time, qty, price
            from collections import defaultdict
            daily_fills = defaultdict(lambda: {'buys': 0, 'sells': 0})
            
            for fill in activities:
                try:
                    # Parse transaction time to get the calendar date
                    tx_time = fill.transaction_time
                    if isinstance(tx_time, str):
                        # Handle ISO format string
                        tx_date = datetime.fromisoformat(tx_time.replace('Z', '+00:00'))
                        # Convert to Eastern time for correct calendar day
                        tx_date = tx_date.astimezone(EST).date()
                    else:
                        # Already a datetime object
                        try:
                            tx_date = tx_time.astimezone(EST).date()
                        except:
                            tx_date = tx_time.date() if hasattr(tx_time, 'date') else None
                    
                    if tx_date is None:
                        continue
                    
                    symbol = fill.symbol
                    side = fill.side.lower() if hasattr(fill, 'side') else ''
                    
                    key = (tx_date, symbol)
                    if 'buy' in side:
                        daily_fills[key]['buys'] += 1
                    elif 'sell' in side:
                        daily_fills[key]['sells'] += 1
                    
                except Exception as e:
                    logger.debug(f"Error parsing fill: {e}")
                    continue
            
            # Identify day trades: same ticker bought AND sold on same day
            day_trades = []
            for (trade_date, symbol), counts in daily_fills.items():
                if counts['buys'] > 0 and counts['sells'] > 0:
                    # Each buy+sell pair is one day trade
                    num_day_trades = min(counts['buys'], counts['sells'])
            ...
            # Sort by date
            day_trades.sort(key=lambda x: x['trade_date'])
            
            logger.debug(f"Alpaca fill scan: found {len(day_trades)} day trade(s) in last 10 days")
            return day_trades
            
        except Exception as e:
            logger.warning(f"Could not query Alpaca fill history: {e}")
            return []

    def _get_pdt_countdown_display(self) -> list:
        """Build PDT countdown lines for display — 100% ALPACA-SOURCED.
        
        Alpaca's rolling window = 5 business days.
        A day trade on date X counts through X + 4 business days.
        It falls off on business day X + 5 (i.e. the 6th business day from X).
        ...
        # Get ACTUAL day trades from Alpaca's fill history
        day_trades = self._get_day_trades_from_alpaca()
        
        # Filter to trades still in the 5-business-day window
        active_trades = []
        for trade in day_trades:
            trade_date = trade['trade_date']
            
            # A trade expires after 5 business days from its date
            expiry_date = self._add_business_days(trade_date, 5)
            
            if expiry_date > today:
                bdays_left = self._count_business_days_between(today, expiry_date)
        ...
        lines.append(f"PDT WINDOW STATUS (source: Alpaca):")
        lines.append(f"  Alpaca count: {alpaca_count}/3 used | {remaining} available")
        
        if active_trades:
            # Sort by expiry (soonest first)
            active_trades.sort(key=lambda x: x['expiry_date'])
            
            lines.append(f"  Day trades in rolling 5-day window:")
            for t in active_trades:
                ticker = t['ticker']
                tdate = t['trade_date'].strftime('%a %b %d')
                expiry = t['expiry_date'].strftime('%a %b %d')
                bdays = t['business_days_left']
                lines.append(f"    • {ticker} (traded {tdate}) → clears {expiry} ({bdays} business day{'s' if bdays != 1 else ''} left)")
            
            # Next slot opening = earliest single trade expiry
            earliest_expiry = active_trades[0]['expiry_date']
            earliest_bdays = active_trades[0]['business_days_left']
            
            if alpaca_count >= 3:
                lines.append(f"  Next slot opens: {earliest_expiry.strftime('%A %b %d')} ({earliest_bdays} business day{'s' if earliest_bdays != 1 else ''})")
            
            # Full reset = when ALL active trades have expired
            latest_expiry = max(t['expiry_date'] for t in active_trades)
            full_reset_bdays = self._count_business_days_between(today, latest_expiry)
            lines.append(f"  Full PDT reset (3/3): {latest_expiry.strftime('%A %b %d')} ({full_reset_bdays} business day{'s' if full_reset_bdays != 1 else ''})")
        
        elif alpaca_count > 0:
            # Alpaca shows a count but our fill scan didn't find the trades
            # (possible if trades happened >10 days ago but still in window, or API issue)
            lines.append(f"  ⚠️ Alpaca reports {alpaca_count} day trade(s) but fill history unavailable")
            lines.append(f"  Day trades expire within 5 business days of execution")
            lines.append(f"  Monitor Alpaca dashboard for exact dates")
        
        else:
            lines.append(f"  ✅ No day trades in rolling window — full 3/3 available!")
    ...
    def _run_cycle(self):
        ...
        # v10.1: Query ACTUAL Alpaca PDT count — this is the SOURCE OF TRUTH
        # Our internal counter was blind to rolling 5-day window
        alpaca_count = self._get_alpaca_daytrade_count()
        remaining_day_trades = max(0, 3 - alpaca_count)
        ...
        if remaining_day_trades <= 0:
            ...
            # v10.2: Show countdown only on first cycle or every 30 cycles to avoid spam
            if self.cycle_count == 0 or self.cycle_count % 30 == 0:
                for line in self._get_pdt_countdown_display():
                    pprint(f"  {line}")
